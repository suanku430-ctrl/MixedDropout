...
class Cifar100VGG(nn.Module):
    def __init__(self, dropout_params=None):
        ...
        self.fc_layers = self._build_fc_layers(64 * 8 * 8)
    ...
    def _build_fc_layers(self, in_features):
        modules = []
        ...
        modules.append(nn.Linear(in_features, 1024))
        ...
        modules.append(nn.Linear(1024, 100))
...
def setup_experiment(exp_id, dropout_params=None, lr=0.0001):
    ...
    # Generate parameter string for uniqueness
    param_str = f"lr{lr}"
    if dropout_params:
        # Simplify param string
    ...
    base_dir = "experiments/cifar100/rate_in"
    ...
    logger = logging.getLogger(f'ratein_exp_{exp_id}_{timestamp}')
    logger.setLevel(logging.INFO)

    if logger.hasHandlers():
        logger.handlers.clear()

    file_handler = logging.FileHandler(os.path.join(exp_dir, "console_output.log"))
...
def evaluate(model, test_loader, criterion, device):
    ...
    f1, prec, recall = calculate_metrics(all_targets, all_preds)
    
    return avg_loss, acc, test_time, f1, prec, recall

def run_experiment(exp_id, dropout_params=None, num_epochs=200, lr=0.0001):
    # Setup
    ...
    logger.info(f"Starting Experiment {exp_id}. Logs: {paths['exp_dir']}")

    model = Cifar100VGG(dropout_params=dropout_params).to(device)
    optimizer = optim.RMSprop(model.parameters(), lr=lr)
    criterion = nn.CrossEntropyLoss()


    flops = calculate_flops(model, device=device)
    logger.info(f"Model FLOPs: {flops:,.0f}")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        logger.info(f"Epoch {epoch:3d}/{num_epochs} | "
                    f"Train Loss: {train_loss:.4f} Acc: {train_acc:.2f}% ({train_time:.1f}s) | "
                    f"Test Loss: {test_loss:.4f} Acc: {test_acc:.2f}% ({test_time:.1f}s) | "
    ...
    logger.info(f"Experiment {exp_id} Finished. Best Acc: {best_acc:.2f}%")
    
    # Close logger handlers to prevent memory leaks in loop
...
def main():
    parser = argparse.ArgumentParser(description='Rate-In Dropout CIFAR-100 (Modified)')
    parser.add_argument('--num_experiments', type=int, default=1, help='Repeats')
    parser.add_argument('--num_epochs', type=int, default=100, help='Epochs per run')
    ...
    print("="*30)
    print(f"Results saved in: experiments/cifar100/rate_in/")
